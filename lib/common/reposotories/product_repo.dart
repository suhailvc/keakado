...
class ProductRepo {
  ...
  Future<ApiResponseModel> getAllProductList(
      int? offset, ProductFilterType type) async {
    try {
  ...
  Future<ApiResponseModel> getProductDetails(
      String productID, bool searchQuery) async {
    try {
      ...
      final response =
          await dioClient!.get('${AppConstants.productDetailsUri}$params');
      return ApiResponseModel.withSuccess(response);
    } catch (e) {
      return ApiResponseModel.withError(ApiErrorHandler.getMessage(e));
    }
  ...
  Future<ApiResponseModel> getFlashDeal(int offset) async {
    try {
      final response = await dioClient!
          .get('${AppConstants.flashDealUri}?limit=10&&offset=$offset');
      return ApiResponseModel.withSuccess(response);
    } catch (e) {
      return ApiResponseModel.withError(ApiErrorHandler.getMessage(e));
    }
