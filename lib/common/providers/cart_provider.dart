...
class CartProvider extends ChangeNotifier {
  ...
  void addRemarks(int index, String? newRemarks) {
    if (index >= 0 && index < _cartList.length) {
      if (newRemarks == null || newRemarks.isEmpty) {
        // If new remarks are null or empty, remove the existing remarks
        _cartList[index].remarks = null;
      } else {
        // Add new remarks or replace existing ones
        _cartList[index].remarks = newRemarks;
      }

      cartRepo?.addToCartList(_cartList); // Persist updated cart list
