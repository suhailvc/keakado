    ...
    return CustomDirectionalityWidget(
      ...
      text: TextSpan(
        children: [
          ...
          if ((cart.discountedPrice ?? 0) < (cart.price ?? 0) && isUnitPrice)
            TextSpan(
              style: poppinsRegular.copyWith(
              ...
              text: PriceConverterHelper.convertPrice(context,
                  (cart.price ?? 0) * (isUnitPrice ? 1 : cart.quantity ?? 1)),
            ),
          if ((cart.discountedPrice ?? 0) < (cart.price ?? 0) && isUnitPrice)
            const TextSpan(text: '  '),
          TextSpan(
