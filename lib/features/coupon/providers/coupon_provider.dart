    ...
    if (apiResponse.response != null && apiResponse.response!.data != null) {
      ...
      if (_coupon!.minPurchase != null && _coupon!.minPurchase! <= order) {
        if (_coupon!.discountType == 'percent' &&
          ...
          if (order > AppConstants.mimimumOrderValue) {
            showCustomSnackBarHelper('Already Have Free Delivery',
                isError: true);
          } else {
            freeDeliveryCoupon = true;
            showCustomSnackBarHelper('you_got_free_delivery_offer'.tr,
                isError: false);
          }
        } else {
          _discount = _coupon!.discount;
          showCustomSnackBarHelper(
              '${'you_got_discount'.tr} ${PriceConverterHelper.convertPrice(Get.context!, _coupon!.discount)}',
              isError: false);
        }
      } else {
        showCustomSnackBarHelper('${'you_need_order_more_than'.tr} '
