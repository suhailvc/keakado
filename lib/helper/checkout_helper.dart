...
class CheckOutHelper {
  static List<PaymentMethod> getActivePaymentList(
    ...
    if (configModel.offlinePayment!) {
      paymentMethodList.add(PaymentMethod(
          getWay: 'offline_payment', getWayImage: Images.walletPayment));
    }
  ...
  static double getDeliveryCharge({
    ...
    final isFreeDeliveryEligible = freeDeliveryStatus &&
        (orderAmount + discount) > (freeDeliveryOverAmount ?? 0);

    if (isDeliveryDisabled ||
        isFreeDeliveryEligible ||
        freeDeliveryType == 'free_delivery') {
      deliveryCharge = 0;
    }
